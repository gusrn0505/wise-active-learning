    ...
    def sample(self, already_selected, sample_size):
        ...
        for _ in range(sample_size):
            if self.already_selected == []:
                ...
                ind = np.argmax(self.min_distances) # index of min_distances
            
            # assert ind not in already_selected
            if ind in already_selected : 
                left = [p for p in np.arange(len(self.already_selected), self.dset_size) if p not in new_batch]
                ind = np.random.choice(left)
